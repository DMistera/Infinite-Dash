...
public class Difficulty {
    ...
    public void Set(DifficultyType type, float v) {
        /*if(map[type] > 0) {
            Debug.Log($"Change from {v - map[type]} on {type}");
        }*/
    ...
    public Difficulty(DifficultyEntry[] entries) {
        foreach (DifficultyEntry entry in entries) {
            map.Add(entry.type, entry.value);
        }
    }
